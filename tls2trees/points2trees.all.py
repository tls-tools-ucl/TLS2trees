...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-tiles', default=5, type=int, help='enlarges the number of tiles i.e. 3x3 or tiles or 5 x 5 tiles')
    ...
    parser.add_argument('--find-stems-height', default=2, type=float, help='height for identifying stems')    
    ...
    parser.add_argument('--add-leaves-voxel-length', default=.5, type=float, help='voxel size when adding leaves')
...
#     params.tmp = glob.glob(os.path.join(params.dir, f'{params.n:03}.*.tmp'))[0]
    ...
    trees = pd.merge(stem_pc, stems[['clstr', 'base', 'red', 'green', 'blue', 'distance']], on='clstr')
    ...
    # write out all trees
    params.base_I, I = {}, 0
    tile_tree_N = {t:0 for t in stem_pc.fn.unique()}
    ...
    for i, b in tqdm(enumerate(in_tile_stem_nodes), 
        ...
        N = trees.loc[trees.base == b].fn.value_counts().index[0]
        I = tile_tree_N[N]
        ply_io.write_ply(os.path.join(params.odir, f'{N:03}_T{I}.leafoff.ply'), trees.loc[trees.base == b])
        params.base_I[b] = os.path.join(params.odir, f'{N:03}_T{I}.leafoff.ply')
        tile_tree_N[N] += 1    
...
#         edges = edges.loc[edges.length <= .5]
        ...
        for lv in lvs.loc[lvs.base.isin(in_tile_stem_nodes)].base.unique():
            ...
            ply_io.write_ply(tn.replace('off', 'on'), stem[['x', 'y', 'z', 'red', 'green', 'blue', 'label', 'base', 'wood', 'distance']])
